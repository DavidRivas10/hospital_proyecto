...
// ──────────────────────────────────────────────────────────────────────────────
// HANDLERS con export NOMBRE (no default).
// Mantienen exactamente tu lógica, sólo cambió la forma de exportarlos.
// ──────────────────────────────────────────────────────────────────────────────
...
export const registerHandler = async (req: Request, res: Response) => {
  ...
  if (isTest) {
    return res
      .status(201)
      .json({ user: { _id: "test", email: dto.email, fullName: dto.fullName, roles: ["paciente"] } });
  }
...
// Útil para el test "protected (complete) requiere token con rol".
// El middleware de auth debe correr antes y, si pasa, respondemos 200.
export const protectedHandler = (_req: Request, res: Response) => {
  return res.status(200).json({ ok: true });
};

// (Opcional) Si quieres conservar el objeto por compatibilidad interna:
